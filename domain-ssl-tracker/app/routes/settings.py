...
from app.auth import require_login, is_logged_in
...
def settings_page(request: Request, db: Session = Depends(get_db), saved: str = "", error: str = "", _: None = Depends(require_login)):
    current = get_all(db)
    return templates.TemplateResponse("settings.html", {
        "request": request, "s": current, "saved": saved, "error": error,
    })
...
def save_settings(
    ...
    alert_from_email: str = Form(""), alert_to_email: str = Form(""),
    ssl_alert_days: str = Form("7"), domain_alert_days: str = Form("30"),
    db: Session = Depends(get_db), _: None = Depends(require_login),
):
    save_all(db, {
        "smtp_host": smtp_host.strip(), "smtp_port": smtp_port.strip() or "587",
        "smtp_user": smtp_user.strip(), "smtp_password": smtp_password.strip(),
        "alert_from_email": alert_from_email.strip(), "alert_to_email": alert_to_email.strip(),
...
def test_email(db: Session = Depends(get_db), _: None = Depends(require_login)):
